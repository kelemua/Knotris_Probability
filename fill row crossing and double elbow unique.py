...
def checkSC(uniRowPermuts):
    """
    suitably connected (SC)
    not suitably connected (NSC)
    """
...
def boundary(uniRowPermutsSC):
    ...
    #reduced BCs composed of single BC from each symmetric pair 
        #For example (0,0,0,0,0,1) and (1,0,0,0,0,0) are a symmetric pair and only (0,0,0,0,0,1) will be displayed
    print('\nReduced Boundary Conditions')      #reduced boundary conditions 
    reducedRowBoundaries = []    
    for boundaryCond in rowBoundaries:
        if (boundaryCond[5], boundaryCond[4], boundaryCond[3], boundaryCond[2], boundaryCond[1], boundaryCond[0]) not in reducedRowBoundaries:
            reducedRowBoundaries.append(boundaryCond)
    for i in reducedRowBoundaries: 
        if i in boundaryCounter:
            print(i, boundaryCounter[i])
        else: 
            print(i, 0)
